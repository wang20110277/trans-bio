...
def prepare_training_data(arabidopsis_data):
    X = one_hot_encode(arabidopsis_data)
    y = np.random.randint(2, size=len(arabidopsis_data))  # 替换为真实标签
    return X, y


def prepare_test_data(cucumber_data):
    return one_hot_encode(cucumber_data)
...
if __name__ == '__main__':
    ...
    arabidopsis_data = load_arabidopsis_data(arabidopsis_data_path)
    cucumber_data = load_cucumber_data(cucumber_data_path)
    
    # 准备训练数据
    X_train, y_train = prepare_training_data(arabidopsis_data)
    
    # 训练模型
    model = train_model(X_train, y_train)
    
    # 准备测试数据
    X_test = prepare_test_data(cucumber_data)
